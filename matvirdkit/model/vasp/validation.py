...
from matvirdkit.model.settings import ( VASP_KPTS_TOLERANCE,
VASP_KSPACING_TOLERANCE,
VASP_DEFAULT_INPUT_SETS,
VASP_CHECKED_LDAU_FIELDS,
VASP_MAX_SCF_GRADIENT)

from matvirdkit.model.utils import DocEnum
...
class ValidationDoc(BaseModel):
    ...
    def from_task_doc(
        ...
        kpts_tolerance: float = VASP_KPTS_TOLERANCE,
        kspacing_tolerance: float = VASP_KSPACING_TOLERANCE,
        input_sets: Dict[str, str] = VASP_DEFAULT_INPUT_SETS,
        #input_sets: Dict[str, PyObject] = VASP_DEFAULT_INPUT_SETS,
        LDAU_fields: List[str] = VASP_CHECKED_LDAU_FIELDS,
        max_allowed_scf_gradient: float = VASP_MAX_SCF_GRADIENT,
    ) -> "ValidationDoc":
        """
...
if __name__ == '__main__':
   td1=TaskDocument.from_directory(task_id='t-1',task_dir='calc_types/relax',dst_dir='output')
   vd = ValidationDoc.from_task_doc(td1)
   print(vd)
